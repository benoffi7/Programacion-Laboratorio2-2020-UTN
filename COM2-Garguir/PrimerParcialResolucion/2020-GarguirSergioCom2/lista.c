...
void muestaListaRecursiva(nodo* lista){
    if(lista){
        muestraNodo(lista);
        muestaListaRecursiva(lista->siguiente);
    }
}
...
nodo* agregarEnOrdenApellido(nodo* lista, nodo* nuevo){
    if(lista==NULL){
        ...
        if(atoi(nuevo->dato.apellido)<atoi(lista->dato.apellido)){
            lista=agregarAlPrincipio(lista, nuevo);
        }else{
            nodo* ante = lista;
            nodo* seg = lista->siguiente;
            while(seg && (atoi(nuevo->dato.apellido)>atoi(seg->dato.apellido))){
                ante = seg;
                seg = seg->siguiente;
            }
            nuevo->siguiente = seg;
...
nodo* borrarNodo(nodo* lista, char apellido[]){
    ...
    if(lista && strcmp(lista->dato.apellido, apellido)==0){
        ...
        seg = lista;
        while(seg && strcmp(lista->dato.apellido, apellido)!=0){
            ante = seg;
            seg = seg->siguiente;
        }
        if(seg){
